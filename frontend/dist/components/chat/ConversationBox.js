...
export const ConversationBox = defineComponent({
  ...
  render() {
    ...
    return h("div", {
      ...
      id: "chat-container"
    }, loading ? h("p", null, "Loading messages...") : error ? h("p", null, "Error: ", error) : h("div", {
      className: "h-100 d-flex flex-column"
    }, h("div", {
      className: "flex-grow-1"
    }), messages.map(message => message.sender === "user" ? message.message_type === 0 ? h(SentMessage, {
      message: {
        text: message.text,
        time: message.time
      }
    ...
    }) : message.message_type === 0 ? h(ReceivedMessage, {
      message: {
        text: message.text,
        time: message.time
      }
    }) : h(ReceivedInvitation, {
      message: message,
      "on:acceptInvitation": () => this.acceptInvitation(message)
    })))), conversation.isBlocked ? h(Fragment, null) : h(ChatInput, {
  ...
  methods: {
    ...
    handleScroll() {
      ...
      if (messageContainer) {
        ...
        this.state.shouldScrollToBottom = scrollHeight - scrollTop - clientHeight < 100;
      }
    },
    ...
    onUserBlock(data) {
      ...
      if (conversation.friend_id !== blocker_id && conversation.friend_id !== blocked_id) {
        return;
      }
      this.$updateState({
        messages: []
    ...
    async acceptInvitation(message) {
      ...
      const isWithinTenMinutes = Date.parse(message.invitation.created_at) + 10 * 60 * 1000 > Date.now();
      console.log("Date.parse(message.invite.created_at)", Date.parse(message.invitation.created_at));
  ...
  async onMounted() {
    ...
    try {
      const response = await myFetch(`${BACKEND_URL}/manage/chat/api/messages/${conversation.id}/`);
      if (response.ok) {
      ...
      chatWebSocketManager.subscribe("load_previous_messages", this.prevMessagesCallback);
  ...
  onUnmounted() {
    ...
    chatWebSocketManager.unsubscribe("load_previous_messages", this.prevMessagesCallback);
