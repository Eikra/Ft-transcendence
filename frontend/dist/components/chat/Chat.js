...
export const Chat = defineComponent({
  state() {
    return {
      conversations: [],
      // Conversations from the backend
      activeUser: null,
      // Active user ID
  ...
  async onMounted() {
    ...
    try {
      const response = await myFetch(`${BACKEND_URL}/manage/chat/api/conversations/`);
      if (response.ok) {
        const data = await response.json();
        const urlUserId = this.$context.router.params.id;
        // console.log("a- ", this.$context.router.params.id);
        // console.log("b- ", data);
        const selectedConv = data.find(conv => conv.friend_id == this.$context.router.params.id);
        this.$updateState({
          conversations: data,
  ...
  methods: {
    ...
    handleConversationSelected({
      ...
      const targetConversation = updatedConversations.find(conv => conv.id === conversation.id);
    ...
    updateOnlineStatus(updates) {
      const updatedConversations = this.state.conversations.map(conversation => {
        const update = updates.find(u => u.user_id === conversation.friend_id);
        if (update) {
          return {
            ...conversation,
      ...
      const updatedActiveConversation = this.state.activeConversation && updatedConversations.find(conversation => conversation.id === this.state.activeConversation.id);
      this.$updateState({
        conversations: updatedConversations,
        activeConversation: updatedActiveConversation || this.state.activeConversation
      });
    },
    handleMessage(data) {
      ...
      const updatedConversations = this.state.conversations.map(conversation => conversation.id === chat_id ? {
        ...conversation,
        lastMessage,
  ...
  render() {
    ...
    return h("div", {
      ...
      className: "h-100"
    }, loading ? h("p", null, i18n.t("chat.loading")) : error ? h("p", null, i18n.t("chat.error", {
      error: error
    })) : h(ChatMenu, {
    ...
    }, this.state.activeConversation === null || !this.state.socketConnected ? h("div", {
      className: "h-100 d-flex flex-column justify-content-center align-items-center"
    }, h("img", {
      src: "./img/empty-messages.png",
      className: "c-empty-messages-img",
      alt: i18n.t("chat.empty_messages_alt")
    }), h("h3", null, i18n.t("chat.start_message"))) : h(ConversationBox, {
