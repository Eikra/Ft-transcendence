...
export class Game {
  ...
  inintGame(data) {
    ...
    window.addEventListener("keydown", event => {
      const key = event.key;
      if (key == "ArrowUp") this.socket.send(JSON.stringify({
        type: "key_down",
        key: "ArrowUp"
      }));else if (key == "ArrowDown") this.socket.send(JSON.stringify({
        type: "key_down",
        key: "ArrowDown"
      }));
    });
  ...
  drawCircle(ball) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(ball.x * this.scale, ball.y * this.scale, ball.radius * this.scale, 0, 2 * Math.PI);
    this.ctx.stroke();
  ...
  drawPaddle(paddle) {
    this.ctx.fillStyle = "white";
    this.ctx.fillRect(paddle.x * this.scale, paddle.y * this.scale, paddle.width * this.scale, paddle.height * this.scale);
  }
  drawScore() {
    this.ctx.fillStyle = "white";
    ...
    this.ctx.fillText(this.scores.player1, this.canvas.width / 2 - 100 * this.scale - this.ctx.measureText(playerScore).width, 80 * this.scale);
    this.ctx.fillText(this.scores.player2, this.canvas.width / 2 + 100 * this.scale, 80 * this.scale);
  }
  drawBorders() {
    this.ctx.strokeStyle = "white";
