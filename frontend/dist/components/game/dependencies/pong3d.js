...
export class Game {
  ...
  registerEvents() {
    this.socket.onmessage = event => {
      ...
      if (data.type === "game_start") {
        ...
        this.playersInfo.playerFrontAvatar = data.playerFrontData.avatar;
        this.playersInfo.playerBack = data.playerBackData.display_name;
        this.playersInfo.playerFront = data.playerFrontData.display_name;
  ...
  checkAndDisplayObjects() {
    const interval = setInterval(() => {
      if (this.score1Mesh && this.score2Mesh && this.frontPaddle && this.backPaddle && this.ballMesh && this.frontWall && this.backWall && this.rightWall && this.leftWall && this.tableModel && this.frameMesh && this.pictureMesh && this.frameMesh2 && this.pictureMesh2 && this.number3Mesh && this.number2Mesh && this.number1Mesh && this.number0Mesh && this.startMesh && this.VS_Mesh && this.profileBackMesh && this.profileFrontMesh) {
        ...
        const requiredObjects = [this.frameMesh, this.pictureMesh, this.frameMesh2, this.pictureMesh2, this.number3Mesh, this.startMesh, this.VS_Mesh, this.profileBackMesh, this.profileFrontMesh];
  ...
  gameStartFunc() {
    ...
    const requiredObjects = [this.score1Mesh, this.score2Mesh, this.frontPaddle, this.backPaddle, this.ballMesh, this.frontWall, this.backWall, this.rightWall, this.leftWall, this.tableModel];
    const interval2 = setInterval(() => {
      clearInterval(interval2);
      if (this.isGameOver == true) return;
      // Check if all required objects are in the scene
  ...
  adding3dText(data) {
    ...
    startMesh.position.set(-startWidth / 2, this.camera.position.y + 5, this.camera.position.z / 3);
    ...
    VS_Mesh.position.set(-VS_Width / 2, this.camera.position.y - 7, this.camera.position.z / 3);
    ...
    this.profileBackMesh = this.addPlayerName(data.playerBackData.display_name || "Player 1", -10);
    this.profileFrontMesh = this.addPlayerName(data.playerFrontData.display_name || "Player 2", 10);
  }
  get3dNumber(numberText) {
    ...
    numberMesh.position.set(-numberWidth / 2, this.camera.position.y, this.camera.position.z / 3);
    return numberMesh;
  }
  loadingProfiles(data) {
    ...
    const profileImage = loadOptimizedTexture(`${BACKEND_URL}/auth/media/${data.playerFrontData.avatar}`);
    const profileImage2 = loadOptimizedTexture(`${BACKEND_URL}/auth/media/${data.playerBackData.avatar}`);
    ...
    frameMesh.position.set(10, this.camera.position.y - 5, this.camera.position.z / 3);
    pictureMesh.position.set(10, this.camera.position.y - 5, this.camera.position.z / 3);
    ...
    frameMesh2.position.set(-10, this.camera.position.y - 5, this.camera.position.z / 3);
    pictureMesh2.position.set(-10, this.camera.position.y - 5, this.camera.position.z / 3);
  ...
  addCamera() {
    ...
    if (this.canvasParent.clientWidth <= 428) {
      this.camera.position.set(0, 95 + 95 * (40 / this.canvasParent.clientWidth), 46 + 46 * (40 / this.canvasParent.clientWidth));
  ...
  addPlayerName = (name, xPosition) => {
    ...
    nameMesh.position.set(xPosition - nameWidth / 2, this.camera.position.y - 13, this.camera.position.z / 3);
    return nameMesh;
  };
  ...
  keyupHandler(event) {
    const key = event.key;
    if (key == "ArrowRight") this.socket.send(JSON.stringify({
      type: "key_up",
      key: "ArrowRight"
    }));else if (key == "ArrowLeft") this.socket.send(JSON.stringify({
      type: "key_up",
      key: "ArrowLeft"
    }));
  }
  ...
  create3Dobjects() {
    ...
    loader.load("./assets/source/day_6_video_game/scene.gltf", gltf => {
  ...
  addSoundEffects() {
    ...
    this.audioLoader.load("./assets/source/paddle_hit.mp3", buffer => {
    ...
    this.audioLoader.load("./assets/source/goal.mp3", buffer => {
    ...
    this.audioLoader.load("./assets/source/wall_hit.mp3", buffer => {
    ...
    this.audioLoader.load("./assets/source/game-countdown.mp3", buffer => {
  ...
  playingGameOver(isWinner) {
    ...
    let winnerText = null;
    const loader = new TTFLoader();
    if (isWinner == true) winnerText = "YOU WON";else winnerText = "YOU LOST";
    loader.load("./assets/source/kenpixel.ttf", json => {
      const font = new Font(json);
      this.textFont = font;
      ...
      const winnerMesh = new THREE.Mesh(isWinnerGeometry, isWinnerMaterial);

      // Center the text
      isWinnerGeometry.computeBoundingBox();
      const winnerBoundingBox = isWinnerGeometry.boundingBox;
      const winnerWidth = winnerBoundingBox.max.x - winnerBoundingBox.min.x;
      winnerMesh.position.set(-winnerWidth / 2, this.camera.position.y - 5, this.camera.position.z / 3);
      ...
      GameOverMesh.position.set(-GameOverWidth / 2, this.camera.position.y, this.camera.position.z / 3);
