...
export function initLocalPongGame() {
  ...
  // Fetch user data
  myFetch(`${BACKEND_URL}/manage/profile/`).then(response => response.json()).then(userData => {
    // console.log(userData);
    let accessToken = localStorage.getItem("access_token");
    console.log(`token=${accessToken}`);
    socket = new WebSocket(`ws://127.0.0.1:8000/ws/game/local/123/?user_id=${userData.user}`);
    socket.onopen = () => {
    ...
    socket.onmessage = event => {
      ...
      if (data.type === "mouse_move") {
        ...
        console.log("Message from server:", data);
        ball.position.set(data.message.ball_position.x, ball.position.y, data.message.ball_position.z);
        // Position the paddle on the table
        // paddle.rotation.y = Math.PI / 2;
        paddle.position.set(data.message.paddle1_position.x, data.message.paddle1_position.y, data.message.paddle1_position.z); // Adjust the position as needed
        console.log(paddle.position);

        // paddle2.rotation.y = Math.PI / 2;
        paddle2.position.set(data.message.paddle2_position.x, data.message.paddle2_position.y, data.message.paddle2_position.z); // Adjust the position as needed

        game_start = true;
        player = data.player;
        if (player === data.message.player1) opponent = data.message.player2;else opponent = data.message.player1;
        ...
        if (data.message.username === player) {
          ...
          if (player === data.message.shared_state.player1) {
            if (score1 !== data.message.shared_state.score1 && data.message.shared_state.score1 != null) {
              score1 = data.message.shared_state.score1;
              updateScore(1);
            }
            if (score2 !== data.message.shared_state.score2 && data.message.shared_state.score2 != null) {
              score2 = data.message.shared_state.score2;
              updateScore(2);
            }
          } else if (player === data.message.shared_state.player2) {
            if (score1 !== data.message.shared_state.score2 && data.message.shared_state.score2 != null) {
              score1 = data.message.shared_state.score2;
              updateScore(1);
            }
            if (score2 !== data.message.shared_state.score1 && data.message.shared_state.score1 != null) {
        ...
        // console.log(data)
        if (data.message.username === player) {
          if (player === data.message.paddle_state.player1) paddle.position.x = data.message.paddle_state.player1_pos.x;else if (player === data.message.paddle_state.player2) paddle.position.x = data.message.paddle_state.player2_pos.x;
          // paddle.position.x = data.message.paddle_state.player.x;
        }
        //     paddle.position.x = data.paddle_state.player.x;
        else if (data.message.username === opponent) {
          if (opponent === data.message.paddle_state.player1) paddle2.position.x = -data.message.paddle_state.player1_pos.x;else if (opponent === data.message.paddle_state.player2) paddle2.position.x = -data.message.paddle_state.player2_pos.x;
        }
        ...
        if (data.message.username === player) {
          if (data.message.shared_state.winer === player) {
            console.log(`the winer is ${player} and the loser is ${opponent}`);
          } else if (data.message.shared_state.winer === opponent) {
            console.log(`the winer is ${opponent} and the loser is ${player}`);
          }
        } else if (data.message.username === opponent) {
          if (data.message.shared_state.winer === player) {
            console.log(`the winer is ${player} and the loser is ${opponent}`);
  ...
  const camera2 = new THREE.PerspectiveCamera(75, gameBlock.width / gameBlock.height, 0.1, 1000);
  ...
  let score2Mesh = null;
  let ScoreFont = null;
  textloader.load("./assets/source/modes/Pong_Score_Regular.json", function (loadedFont) {
