...
export class localGame {
  ...
  resizeHandler() {
    ...
    } else this.camera.position.set(0, 120, 0);
  ...
  updatePaddeles() {
    ...
    if (this.ball.x + this.ball.speedX <= -16.6 || this.ball.x + this.ball.speedX >= 16.6) {
      this.ball.speedX *= -1;
      this.sound = "wall";
    ...
    if (this.ball.z <= -26) {
      ...
      this.updateSound("goal");
      this.sound = "goal";
    } else if (this.ball.z > 0 && this.ball.z < this.paddles.front.z && Math.abs(this.ball.z - this.paddles.front.z) < 1.25 && Math.abs(this.ball.x - this.paddles.front.x) < 3.75) {
      this.ball.z -= 1.26 - Math.abs(this.ball.z - this.paddles.front.z);
      this.ball.speedZ *= -1;
      let distanceX = Math.abs(this.paddles.front.x - this.ball.x);
      let dx_sign = 0;
      if (this.ball.x > this.paddles.front.x) dx_sign = 1;else dx_sign = -1;
      if (distanceX <= 3) this.ball.speedX = distanceX * 0.5 * this.GAME_SPEED * dx_sign;
      if (this.ball.x + this.ball.speedX <= -16.6 || this.ball.x + this.ball.speedX >= 16.6) this.ball.speedX *= -1;
      this.updateSpeed(0.05);
      this.sound = "paddle";
      this.updateSound("paddle");
    } else if (this.ball.z < 0 && this.ball.z > this.paddles.back.z && Math.abs(this.ball.z - this.paddles.back.z) < 1.25 && Math.abs(this.ball.x - this.paddles.back.x) < 3.75) {
      this.ball.z += 1.26 - Math.abs(this.ball.z - this.paddles.back.z);
      this.ball.speedZ *= -1;
      let distanceX = Math.abs(this.paddles.back.x - this.ball.x);
      let dx_sign = 0;
      if (this.ball.x < this.paddles.back.x) dx_sign = -1;else dx_sign = 1;
      if (distanceX <= 3) this.ball.speedX = distanceX * 0.5 * this.GAME_SPEED * dx_sign;
      if (this.ball.x + this.ball.speedX <= -16.6 || this.ball.x + this.ball.speedX >= 16.6) this.ball.speedX *= -1;
      this.updateSpeed(0.05);
      this.sound = "paddle";
      this.updateSound("paddle");
    } else if (this.sound != "wall") this.sound = "";
  }
  keyupHandler(event) {
    const key = event.key;
    if (key == "ArrowUp") this.paddles.front.dx = 0;else if (key == "ArrowDown") this.paddles.front.dx = 0;else if (key == "W" || key == "w") this.paddles.back.dx = 0;else if (key == "S" || key == "s") this.paddles.back.dx = 0;
  }
  keydownHandler(event) {
    const key = event.key;
    let paddleSpeed = 1.5 * this.GAME_SPEED;
    if (key == "ArrowUp") this.paddles.front.dx = paddleSpeed * 1;else if (key == "ArrowDown") this.paddles.front.dx = paddleSpeed * -1;else if (key == "W" || key == "w") this.paddles.back.dx = paddleSpeed * 1;else if (key == "S" || key == "s") this.paddles.back.dx = paddleSpeed * -1;
  }
  updateBall() {
    if (this.paddles.back.dx != 0 && -17 + this.PADDLE_WIDTH <= this.paddles.back.x + this.paddles.back.dx && this.paddles.back.x + this.paddles.back.dx <= 17 - this.PADDLE_WIDTH) this.paddles.back.x += this.paddles.back.dx;
    // else if (this.paddles.back.dx < 0)
    //     this.paddles.back.x = -17 + this.PADDLE_WIDTH;
    // else if (this.paddles.back.dx > 0)
    //     this.paddles.back.x = 17 - this.PADDLE_WIDTH;

    if (this.paddles.front.dx != 0 && -17 + this.PADDLE_WIDTH <= this.paddles.front.x + this.paddles.front.dx && this.paddles.front.x + this.paddles.front.dx <= 17 - this.PADDLE_WIDTH) this.paddles.front.x += this.paddles.front.dx;
  ...
  updateSpeed(SPEED_INCREMENT) {
    ...
    if (this.GAME_SPEED < MAX_GAME_SPEED) this.GAME_SPEED += SPEED_INCREMENT;
    this.GAME_SPEED = Math.min(this.GAME_SPEED, MAX_GAME_SPEED);
  }
  reset_ball() {
    this.GAME_SPEED = 0.2;
    this.ball.x = 0;
    ...
    let dz_sign = 0;
    if (this.ball.speedZ > 0) dz_sign = 1;else dz_sign = -1;
  ...
  checkAndDisplayObjects() {
    const interval = setInterval(() => {
      if (this.score1Mesh && this.score2Mesh && this.frontPaddle && this.backPaddle && this.ballMesh && this.frontWall && this.backWall && this.rightWall && this.leftWall && this.tableModel && this.number3Mesh && this.number2Mesh && this.number1Mesh && this.number0Mesh && this.startMesh && this.profileBackMesh && this.profileFrontMesh) {
        ...
        const requiredObjects = [this.number3Mesh, this.startMesh];
        if (this.isGameOver == true) return;
        const interval2 = setInterval(() => {
  ...
  clearAllTimers() {
    if (this.checkGameOver) this.intervals.push(this.checkGameOver);
