...
export const GameDisplayer = defineComponent({
  ...
  render() {
    ...
    if (this.state.loading && this.state.gameOver) {
      console.error("Unexpected state: Loading and gameOver simultaneously.");
      return h("div", {
        className: "error-message"
      }, "Something went wrong. Please try again later.");
    }
    if (this.state.invalidInvite) {
      return h(InvalidInvite, null);
    }
    return h("div", {
      className: "h-100 d-flex justify-content-center align-items-center",
      id: "canvasParent"
    }, this.state.hasInvite ? h(InviteLobby, null) : h(Fragment, null), this.state.gameOver ? h(GameOver, {
