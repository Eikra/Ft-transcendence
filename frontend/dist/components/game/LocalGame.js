...
export const LocalGame = defineComponent({
  ...
  render() {
    if (this.state.gameOver) {
      return h("div", {
        className: "d-flex flex-column justify-content-center align-items-center h-100 gap-4"
      }, h("h1", {
        className: "c-user-stat-name"
      }, "The winner is", " ", this.state.playersInfo.playerFrontScore > this.state.playersInfo.playerBackScore ? this.state.playersInfo.playerFront : this.state.playersInfo.playerBack), h("button", {
        className: "btn c-btn",
  ...
  async onMounted() {
    ...
    this.$updateState({
      game: new localGame(canvasParent, this)
    });
  },
  onUnmounted() {
    if (this.state.game) {
