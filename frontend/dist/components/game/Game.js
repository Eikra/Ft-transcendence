...
export const Game = defineComponent({
  ...
  render() {
    ...
    if (this.state.activeComponent === "local-pong") {
      return h(LocalGame, {
        "on:backToMenu": () => this.$updateState({
          activeComponent: "gameIntro"
        })
      });
    }
  ...
  async onMounted() {
    ...
    if (tournamentId) {
      try {
        const response = await myFetch(`${BACKEND_URL}/manage/tour/tournament/game/${id}/auth`);
        if (!response.ok) {
          throw new Error(`Error fetching auth: ${response.statusText}`);
        }
