...
export const SearchBar = defineComponent({
  ...
  render() {
    return h("div", {
      className: joinClasses(this, "position-relative")
    }, h(SearchInput, {
      id: this.props.id,
    ...
    }), this.state.input.length > 1 && this.state.focus && !this.state.isLoading ? h(SearchResult, {
  ...
  methods: {
    async searchUsers() {
      ...
      const response = await myFetch(`${BACKEND_URL}/manage/search-users/?search=${this.state.input}`).catch(error => {
        return this.$updateState({
          error: true,
          isLoading: false
        });
      });
      if (!response.ok) return this.$updateState({
        error: true,
        isLoading: false
      });
