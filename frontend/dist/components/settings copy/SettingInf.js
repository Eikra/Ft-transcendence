...
export const SettingInf = defineComponent({
  ...
  render() {
    return h("div", {
      ...
      "on:inputChange": input => this.$updateState({
      ...
      error: this.state.display_name.length === 0 ? "This faild can't be empty." : null,
      value: this.state.display_name,
      "on:inputChange": input => this.$updateState({
        display_name: input.value
      })
    }), this.props.userData.login_mode === "email" ? h("div", null, h(InputField, {
      type: "password",
      className: "row mb-3 text-start p-2",
      label: "New Password",
    ...
    }, i18n.t("settings.language")), h("select", {
      id: "laguage",
      ...
      value: "fr"
    }, "Fran\xE7ais"), h("option", {
      value: "sp"
    }, "Espa\xF1ol")))), h(TwoFA, {
      user2FA: this.props.userData.is_2fa_enabled
    }), h("div", {
  ...
  methods: {
    updateData() {
      ...
      console.log("updateData -> this.state.password", this.state.password.length);
    ...
    hasError() {
      return this.getError(this.state.first_name) || this.getError(this.state.last_name);
    },
    updateUserInf() {
      this.$updateState({
        toast: h(Toast, {
          ...
          "on:closeToast": () => this.$updateState({
            toast: null
          })
        })
      });
