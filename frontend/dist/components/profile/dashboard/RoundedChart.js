...
export const RoundedChart = defineComponent({
  ...
  render() {
    return h("div", {
      className: ""
    }, h("h4", null, i18n.t("profile.level"), " ", this.state.playerLvl), h("canvas", {
      id: "chartCanvas",
      width: "300",
      height: "300"
    }));
  ...
  onMounted() {
    ...
    const targetEndAngle = startAngle + 2 * Math.PI * (this.state.lvlFractions / 100);
    ...
    const animate = timestamp => {
      ...
      ctx.fillText(`${Math.floor(this.state.lvlFractions * progress)}%`, 150, 150);
