...
export const TournamentLobby = defineComponent({
  ...
  methods: {
    handleStart() {
      ...
      if (participants.length < maxSlots) {
        alert(`Cannot start tournament! Only ${participants.length} out of ${maxSlots} slots are filled.`);
      } else {
  ...
  render() {
    return h("div", {
      className: "tournament-lobby p-4"
    }, [h("div", {
      className: "participants-list d-flex flex-wrap gap-3"
    }, this.props.participants.length > 0 ? this.props.participants.map(participant => h("div", {
      className: "participant-card text-center"
    }, [h("img", {
      ...
      className: "mt-2"
    }, participant.user.username || "Unknown Player")])) : h("p", {}, "No participants have joined yet.")), h("button", {
      className: "btn c-btn-start mt-4",
      "on:click": () => this.handleStart(),
      disabled: this.props.participants.length < this.props.maxSlots // Disable button if slots aren't full
    }, "Start Tournament")]);
  }
});