...
export const TournamentList = defineComponent({
    ...
    async joinTournament(tournamentId) {
      try {
        ...
        if (response.ok) {
          ...
          const updatedTournaments = this.state.tournaments.map(tournament => tournament.id === tournamentId ? {
            ...tournament,
            has_joined: true
          } : tournament);
          this.$updateState({
  ...
  render() {
    ...
    }, this.state.activeComponent === "TournamentsDisplay" && id == null ? h(TournamentsDisplay, {
