...
export const TournamentDetail = defineComponent({
  ...
  async onMounted() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(i18n.t("tournament.detail.errors.fetch_error", {
      ...
      tournamentWebSocketManager.subscribe("participant_list", this.participantListCallBack);
      tournamentWebSocketManager.subscribe("not_allowed", this.handleNotAllowedCallBack);
      if (id && this.state.participants.length === data.tournament.max_players) {
  ...
  onUnmounted() {
    tournamentWebSocketManager.unsubscribe("participant_list", this.participantListCallBack);
    tournamentWebSocketManager.unsubscribe("not_allowed", this.handleNotAllowedCallBack);
    ...
    async startTournament(tournamentId) {
      try {
        ...
        if (response.ok) {
          ...
          //   data
          // );
          notify("success", i18n.t("tournament.detail.notifications.start_success"));
          this.$updateState({
            showRounds: true
          });
        } else {
          console.error(i18n.t("tournament.detail.logs.start_failed"), data.error);
  ...
  render() {
    ...
    if (loading) {
      return h("div", {
        className: "loading"
      }, i18n.t("tournament.detail.loading"));
    }
    if (!tournamentDetail) {
      return h("div", {
        className: "loading"
    ...
    }, h("h2", null, i18n.t("tournament.detail.lobby_title"), ":", " ", tournamentDetail.name || i18n.t("tournament.detail.unnamed")), h("p", null, i18n.t("tournament.detail.available_slots", {
      count: availableSlots
    })), showRounds ? h(TournamentRounds, {
