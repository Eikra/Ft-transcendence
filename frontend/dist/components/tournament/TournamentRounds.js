...
export const TournamentRounds = defineComponent({
  ...
  async onMounted() {
    try {
      ...
      // Fetch current user
      const userResponse = await myFetch(`${BACKEND_URL}/manage/profile/`);
      if (userResponse.ok) {
      ...
      if (tournamentWebSocketManager.socket) {
        tournamentWebSocketManager.socket.send(JSON.stringify({
          type: "user_join_rounds",
          tournament_id: this.props.tournamentId
        }));
        tournamentWebSocketManager.subscribe("rounds_update", this.updateRoundsCallBack);
      }
    } catch (error) {
  ...
  onUnmounted() {
    if (tournamentWebSocketManager) {
      tournamentWebSocketManager.unsubscribe("rounds_update", this.updateRoundsCallBack);
    }
  },
  methods: {
    updateRounds(data) {
    ...
    async handleStartMatch(match) {
      ...
      if (!currentUser || currentUser.user !== match.player_one.id && currentUser.user !== match.player_two.id) {
        alert("Only the players in this match can start it.");
        return;
      }
      try {
        const response = await myFetch(`${BACKEND_URL}/manage/tour/match/${match.id}/start/`, {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          ...
          this.$context.router.navigateTo(`/tournament/${this.props.tournamentId}/game/${data.match_id}`);
        } else {
  ...
  render() {
    ...
    }, [h("p", {}, `${match.player_one.display_name} vs ${match.player_two.display_name}`), h("p", {}, `Status: ${match.status}`), match.status !== "Completed" ? h("button", {
      className: "btn-start-match",
      "on:click": () => this.handleStartMatch(match),
      disabled: this.state.currentUser && this.state.currentUser.user !== match.player_one.id && this.state.currentUser.user !== match.player_two.id
    }, "Start Match") : null, match.winner ? h("p", {}, `Winner: ${match.player_one.id == match.winner ? match.player_one.display_name : match.player_two.display_name}`) : h("p", {}, "Winner: TBD")])))])));
  }
});